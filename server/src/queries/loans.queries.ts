...
export const attemptInstalments = async (
  ...
  for (const { account_number } of accountsWithLoans) {
    ...
    const instalTotal = totalOutstanding * instalmentPercentage;
    ...
    // Attempt repayments for each loan
    for (const loan of sortedLoans) {
      const instalment = +(loan.initial_amount * instalmentPercentage).toFixed(2);

      if (instalment <= 0 || instalment > loan.outstanding_amount) continue;

      await repayLoan(loan.loan_number, account_number, instalment);
    }
  }
  });
};
